...
'''
  ...
  @api_view(['GET'])
  @permission_classes((permissions.IsAuthenticated))
  def get_all_agents(req):
    agents = Profile.objects.filter(is_agent=True)
    serializer = ProfileSerializer(agent)
...
class GetProfileAPIView(APIView):
  ...
  def get(self, req):
    ...
    serializer = ProfileSerializer(user_profile, context={'req': req})
...
class UpdateProfileAPIView(APIView):
  ...
  def patch(self, req, username):
    ...
    serializer = UpdateProfileSerializer(
      instance=req.user.profile, data=data, partial=True)
